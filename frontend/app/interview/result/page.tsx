    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-black min-h-[calc(100vh-4rem)]">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm text-neutral-300">Synthesizing interview feedback...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex-1 flex bg-black min-h-[calc(100vh-4rem)] relative">
        ...
        <div className="flex-1 p-8 max-w-xl mx-auto flex flex-col items-center justify-center text-center space-y-4">
          ...
          <h2 className="text-xl font-bold text-white">No Interview Session Found</h2>
          <p className="text-xs text-neutral-400">
            No completed evaluation report was found for this session ID. Complete an interview round to receive your AI feedback.
          </p>
          <Link
            href="/interview"
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="card-surface p-5 flex flex-col justify-between bg-[#0E0E14] border border-white/[0.08] rounded-xl">
            ...
            <div className="w-full bg-white/[0.06] rounded-full h-1.5 mt-3 overflow-hidden">
              <div className="bg-purple-500 h-1.5 rounded-full" style={{ width: `${report.technical}%` }} />
            </div>
          </div>
        ...
        <div className="card-surface p-6 space-y-4 bg-[#09090E]/90 border border-white/[0.08] rounded-2xl">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
            {report.recommendations.map((rec, idx) => (
              <div key={idx} className="bg-[#0E0E14] p-3.5 rounded-xl border border-white/[0.07] flex flex-col justify-between">
                <div>
                  <span className="text-[10px] font-bold uppercase tracking-wider text-purple-400">
                    Action Item {idx + 1}
                  </span>
                  <p className="text-xs text-neutral-200 mt-1 font-medium">{rec}</p>
                </div>
                <Link
                  href="/chat"
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-3">
            {report.questionReviews.map((q, idx) => {
              ...
              return (
                <div key={q.questionId || idx} className="card-surface overflow-hidden bg-[#09090E]/90 border border-white/[0.08] rounded-xl">
                  <button
                        ...
                        <div>
                          ...
                          <ul className="space-y-1">
                            {q.idealAnswerHighlights.map((hl, hIdx) => (
                              <li key={hIdx} className="flex items-center gap-2 text-neutral-300">
    ...
    <Suspense
      fallback={
        <div className="flex-1 flex items-center justify-center bg-black min-h-screen">
          <div className="text-center space-y-3">
            <div className="w-12 h-12 border-4 border-purple-500 border-t-transparent rounded-full animate-spin mx-auto" />
            <p className="text-sm text-neutral-300">Loading interview evaluation results...</p>
          </div>
        </div>
      }
    >
